...
class COCO:
    def __init__(self, annotation_file=None):
        ...
        if not annotation_file == None:
            ...
            self.createIndex_msrvtt()
    def createIndex_msrvtt(self):
        vidToAnns={ann['video_id']:[] for ann in self.dataset['sentences']}
        anns=    {ann['sen_id']:[] for ann in self.dataset['sentences']}
        for ann in self.dataset['sentences']:
            vidToAnns[ann['video_id']] +=[ann]
            anns[ann['sen_id']] = ann
        vids= {vid['id'] : {} for vid in self.dataset['videos']}
        for vid in self.dataset['videos']:
            vids[vid['id']] = vid
        cats =[]
        catToVids=[]
        self.anns = anns
        self.imgToAnns = vidToAnns
        self.catToImgs = catToVids
        self.imgs = vids
        self.cats = cats
    ...
    def createIndex(self):
        ...
        imgToAnns = {ann['vid_id']: [] for ann in self.dataset['annotations']}
        anns =      {ann['id']:       [] for ann in self.dataset['annotations']}
        for ann in self.dataset['annotations']:
            imgToAnns[ann['vid_id']] += [ann]
            anns[ann['id']] = ann
    ...
    def showAnns(self, anns):
        ...
        if self.dataset['type'] == 'captions':
            for ann in anns:
                print (ann['caption'])
    def loadRes_msrvtt(self,resFile):
        res=COCO()
        res = COCO()
        res.dataset['images'] = [img for img in self.dataset['videos']]
        ...
        annsImgIds = [int(ann['vid_id']) for ann in anns]
        print(set(self.getImgIds()))
        print(set(annsImgIds))
        assert set(annsImgIds) == (set(annsImgIds) & set(self.getImgIds())), \
               'Results do not correspond to current coco set'
        if 'caption' in anns[0]:
            imgIds = set([img['id'] for img in res.dataset['images']]) & set([ann['vid_id'] for ann in anns])
